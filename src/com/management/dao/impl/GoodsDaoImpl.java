...
public class GoodsDaoImpl implements GoodsDao {
	...
	public List<Goods> queryAllGoods() throws SQLException {
		...
		String sql = "SELECT goods.id,goods.`name`,type.`name`,goods.img,goods.price,goods.is_show FROM goods,type WHERE goods.type_id=type.id AND goods.is_deleted = 0";
	...
	public Goods queryGoodsById(Integer id) throws SQLException {
		...
		String sql = "SELECT goods.id,goods.`name`,type.`name`,goods.img,goods.price,goods.is_deleted,goods.is_show FROM goods,type WHERE goods.type_id=type.id AND goods.is_deleted = 0 AND goods.`id`='"
				+ id + "'";
	...
	public void addGoods(Goods goods) throws SQLException {
		...
		String sql = "INSERT INTO `goods` (`name`, `type_id`, `img`, `price`, `is_deleted`,`is_show`) VALUES (?,?,?,?,?,?)";
	...
	public void alertGoods(Goods goods) throws SQLException {
		...
		String sql = "UPDATE `goods` SET `name`=?, `type_id`=?, `img`=?, `price`=?, `is_show`=? WHERE (`id`=?)";
		Connection con = MySQLConnectionUtils.mySQLConnection();
		PreparedStatement ps = con.prepareStatement(sql);
